...
public abstract class UserCommandSolver {
        ...
        if (user != null) {
            switch (action.getType()) {
                case "favorite":
                    if (user.checkViewed(action.getTitle())) {
                        if (!user.checkFavorite(action.getTitle())) {
                            ...
                            result = String.format("success -> %s was added as favourite",
                                    action.getTitle());
                        } else {
                            result = String.format("error -> %s is already in favourite list",
                                    action.getTitle());
                        }
                    } else {
                ...
                case "view":
                    ...
                    result = String.format("success -> %s was viewed with total views of %d",
                            action.getTitle(), views);
                    break;
                case "rating":
                    ...
                    if (user.checkViewed(action.getTitle())) {
                        if (!user.checkRated(action.getTitle(), action.getSeasonNumber())) {
                            ...
                            result = String.format("success -> %s was rated with %.1f by %s",
                                    action.getTitle(),
                                    action.getGrade(), user.getUsername()
                            );
                        } else {
                            result = String.format("error -> %s has been already rated",
                                    action.getTitle());
                        }
                    } else {
                        result = String.format("error -> %s is not seen",
                                action.getTitle());
                    }
                    break;
                default:
